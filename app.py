...
from models import (showing, Artist, Venue, app, db)
...
def search_venues():
    ...
    return render_template('pages/search_venues.html', results=desired_data,
                           search_term=request.form.get('search_term', ''), count_of_result=count_of_result)
...
def create_venue_submission():
    ...
    venue_data = Venue(name=name, city=city, state=state, address=address, phone=phone, genres=genres,
                       seeking_venue=seeking_venue,
                       seeking_description=seeking_description, website=website, facebook_link=facebook_link,
...
def search_artists():
    ...
    return render_template('pages/search_artists.html', results=desired_data,
                           search_term=request.form.get('search_term', ''), count_of_result=count_of_result)
...
def create_artist_submission():
    ...
    artist_data = Artist(name=name, city=city, state=state, phone=phone, genres=genres, seeking_venue=seeking_venue,
                         seeking_description=seeking_description, website=website, facebook_link=facebook_link,
                         image_link=image_link)
...
def shows():
    ...
    showing_artists = Artist.query.join(showing).join(Venue).filter(
        (showing.c.artist_id == Artist.id) & (showing.c.venue_id == Venue.id)).all()

    showing_venues = Venue.query.join(showing).join(Artist).filter(
        (showing.c.artist_id == Artist.id) & (showing.c.venue_id == Venue.id)).all()

    result = db.session.query(Venue.id, Venue.name, Artist.id, Artist.name, Artist.image_link).filter(
        showing.c.artist_id == Artist.id).filter(showing.c.venue_id == Venue.id).all()

    start_time_arr = []
    for i in range(0, len(result)):
        query = sqlalchemy.text(
            f"SELECT start_time from showing WHERE (venue_id = {result[i][0]} AND artist_id = {result[i][2]})")
        query_result = db.engine.execute(query)
        fetch_all_data = query_result.fetchall()
        for row in fetch_all_data:
            for data in row:
...
def create_show_submission():
    ...
    if artist and venue:
        ...
        sql_query = sqlalchemy.text(
            f"UPDATE showing SET start_time='{start_time}' WHERE (artist_id={artist_id} AND venue_id={venue_id})")
